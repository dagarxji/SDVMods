...
$projectFile = Join-Path $projectDir 'AutoEatEideeBridge.csproj'
...
Write-Host 'Auto Eat Eidee Bridge - Windows build'
Write-Host '--------------------------------'
...
try {
    ...
    if ($DeployToVortex) {
        ...
        $stagingRoot = Get-ChildItem -LiteralPath $VortexModsPath -Directory -Filter 'AutoEatEideeBridge*' |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 1 -ExpandProperty FullName

        if (-not $stagingRoot) {
            $stagingRoot = Join-Path $VortexModsPath "AutoEatEideeBridge $modVersion"
        }

        $targets = @((Join-Path $stagingRoot 'AutoEatEideeBridge'))

        $gameModDir = Join-Path $GameModsPath 'AutoEatEideeBridge'
        ...
        foreach ($target in $targets) {
            ...
            Copy-Item -LiteralPath (Join-Path $outputDir 'AutoEatEideeBridge.dll') -Destination $target -Force
