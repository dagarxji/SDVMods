...
$projectFile = Join-Path $projectDir 'CoopHatchAlert.csproj'
...
Write-Host 'Coop Hatch Alert - Windows build'
Write-Host '--------------------------------'
...
try {
    ...
    if ($DeployToVortex) {
        ...
        $stagingRoot = Get-ChildItem -LiteralPath $VortexModsPath -Directory -Filter 'CoopHatchAlert*' |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 1 -ExpandProperty FullName

        if (-not $stagingRoot) {
            $stagingRoot = Join-Path $VortexModsPath "CoopHatchAlert $modVersion"
        }

        $targets = @((Join-Path $stagingRoot 'CoopHatchAlert'))

        $gameModDir = Join-Path $GameModsPath 'CoopHatchAlert'
        ...
        foreach ($target in $targets) {
            ...
            Copy-Item -LiteralPath (Join-Path $outputDir 'CoopHatchAlert.dll') -Destination $target -Force
