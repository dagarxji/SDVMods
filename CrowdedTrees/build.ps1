...
$projectFile = Join-Path $projectDir 'CrowdedWildTrees.csproj'
...
Write-Host 'Crowded Wild Trees - Windows build'
Write-Host '--------------------------------'
...
try {
    ...
    if ($buildExitCode -ne 0) {
        ...
        Write-Host "If Stardew Valley wasn't detected, configure GamePath for CrowdedWildTrees.ModBuildConfig."
    ...
    if ($DeployToVortex) {
        ...
        $stagingRoot = Get-ChildItem -LiteralPath $VortexModsPath -Directory -Filter 'CrowdedWildTrees*' |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 1 -ExpandProperty FullName

        if (-not $stagingRoot) {
            $stagingRoot = Join-Path $VortexModsPath "CrowdedWildTrees $modVersion"
        }

        $targets = @((Join-Path $stagingRoot 'CrowdedWildTrees'))

        $gameModDir = Join-Path $GameModsPath 'CrowdedWildTrees'
        ...
        foreach ($target in $targets) {
            ...
            Copy-Item -LiteralPath (Join-Path $outputDir 'CrowdedWildTrees.dll') -Destination $target -Force
