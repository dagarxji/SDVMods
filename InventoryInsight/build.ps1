...
$projectFile = Join-Path $projectDir 'InventoryInsight.csproj'
...
Write-Host 'Inventory Insight - Windows build'
Write-Host '--------------------------------'
...
try {
    ...
    if ($DeployToVortex) {
        ...
        $stagingRoot = Get-ChildItem -LiteralPath $VortexModsPath -Directory -Filter 'InventoryInsight*' |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 1 -ExpandProperty FullName

        if (-not $stagingRoot) {
            $stagingRoot = Join-Path $VortexModsPath "InventoryInsight $modVersion"
        }

        $targets = @((Join-Path $stagingRoot 'InventoryInsight'))

        $gameModDir = Join-Path $GameModsPath 'InventoryInsight'
        ...
        foreach ($target in $targets) {
            ...
            Copy-Item -LiteralPath (Join-Path $outputDir 'InventoryInsight.dll') -Destination $target -Force
