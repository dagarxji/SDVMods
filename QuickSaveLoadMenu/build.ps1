...
$projectFile = Join-Path $projectDir 'QuickSaveLoadMenu.csproj'
...
Write-Host 'Quick Save Load Menu - Windows build'
Write-Host '--------------------------------'
...
try {
    ...
    if ($DeployToVortex) {
        ...
        $stagingRoot = Get-ChildItem -LiteralPath $VortexModsPath -Directory -Filter 'QuickSaveLoadMenu*' |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 1 -ExpandProperty FullName

        if (-not $stagingRoot) {
            $stagingRoot = Join-Path $VortexModsPath "QuickSaveLoadMenu $modVersion"
        }

        $targets = @((Join-Path $stagingRoot 'QuickSaveLoadMenu'))

        $gameModDir = Join-Path $GameModsPath 'QuickSaveLoadMenu'
        ...
        foreach ($target in $targets) {
            ...
            Copy-Item -LiteralPath (Join-Path $outputDir 'QuickSaveLoadMenu.dll') -Destination $target -Force
