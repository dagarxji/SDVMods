...
Write-Host 'SeasonFlexibleCommunityCenter - Windows build'
Write-Host '--------------------------------'
...
try {
    ...
    if ($DeployToVortex) {
        ...
        $stagingRoot = Get-ChildItem -LiteralPath $VortexModsPath -Directory -Filter 'SeasonFlexibleCommunityCenter*' |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 1 -ExpandProperty FullName

        if (-not $stagingRoot) {
            $stagingRoot = Join-Path $VortexModsPath "SeasonFlexibleCommunityCenter $modVersion"
        }

        $targets = @((Join-Path $stagingRoot 'SeasonFlexibleCommunityCenter'))

        $gameModDir = Join-Path $GameModsPath 'SeasonFlexibleCommunityCenter'
        ...
        foreach ($target in $targets) {
            ...
            Copy-Item -LiteralPath (Join-Path $outputDir 'SeasonFlexibleCommunityCenter.dll') -Destination $target -Force
