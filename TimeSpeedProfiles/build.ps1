...
$projectFile = Join-Path $projectDir 'TimeSpeedProfiles.csproj'
...
Write-Host 'Time Speed Profiles - Windows build'
Write-Host '--------------------------------'
...
try {
    ...
    if ($DeployToVortex) {
        ...
        $stagingRoot = Get-ChildItem -LiteralPath $VortexModsPath -Directory -Filter 'TimeSpeedProfiles*' |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 1 -ExpandProperty FullName

        if (-not $stagingRoot) {
            $stagingRoot = Join-Path $VortexModsPath "TimeSpeedProfiles $modVersion"
        }

        $targets = @((Join-Path $stagingRoot 'TimeSpeedProfiles'))

        $gameModDir = Join-Path $GameModsPath 'TimeSpeedProfiles'
        ...
        foreach ($target in $targets) {
            ...
            Copy-Item -LiteralPath (Join-Path $outputDir 'TimeSpeedProfiles.dll') -Destination $target -Force
